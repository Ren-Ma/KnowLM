...
def write(args):
    ...
    f_idx_out.write(np.array(start, dtype=np.uint64).tobytes(order='C'))  # 先存起始位置
    f_idx_out.write(np.array(length, dtype=np.uint16).tobytes(order='C')) # 再存长度
    f_idx_out.close()
...
if __name__ == '__main__':
    ...
    if args.mode.lower() == "read":
        ...
        args.read_path_prefix = "/data/renma/unigpt/law_data/preproc刑法"
        read(args)
        ...
        # args.file_path = "/data/renma/unigpt/law_data/刑法.txt"
        # args.file_path = "/data/renma/unigpt/law_data/wenshu5w/管辖案件/data.jsonl"
        args.save_prefix = "data"
        # args.save_path = "/data/renma/unigpt/law_data/wenshu5w/管辖案件/"
        args.language = "chinese"
        args.do_split_sentences = True
        args.do_keep_newlines = True
        args.seq_length = 1024
        args.num_workers = 1
        args.tokenizer_path = "/data/caihua/huggingfaceModels/llama/llama-13B/tokenizer.model"
        wenshupath = "/data/renma/unigpt/law_data/wenshu5w/"
        ...
        subdirectories = get_subdirectories(wenshupath)
        for subdir in subdirectories:
            args.file_path = os.path.join(subdir, "data.jsonl")
            args.save_path = subdir
            write(args)
            print(subdir+"处理完成！")
        ...
        args.merge_path_prefix="['民事案件data', '刑事案件data', '行政案件data', '执行案件data', '赔偿案件data']" 
        args.merge_path_type=[0,0,0,0,0] 
        args.new_path_prefix="/data/renma/unigpt/law_data/wenshu5w/merged"
        merge(args)
    else:
        assert False
